  ...
  async store(req, res) {
    ...
    if (!qtdContainers) {
      return res
        .status(400)
        .json(
          'Está faltando o atributo "qtdContainers" no corpo da requisição'
        );
    }
    ...
    if (configAtual[0]) {
      ...
      return res
        .status(200)
        .json(
          `Quantidade de containers atualizada. Nova capacidade: ${qtdContainers}`
        );
    } else {
      ...
      return res
        .status(200)
        .json(`Quantidade inserida. Capacidade: ${qtdContainers}`);
    }
  }
};
